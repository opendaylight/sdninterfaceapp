...
public class LinkstateNlriParserTest {
	private static final Logger LOG = LoggerFactory.getLogger(LinkstateNlriParserTest.class);
    ...
    public void testLinkNlri() throws BGPParsingException {
        ...
        String sdniData = "Sample SDNI Data from the Rest API";
        buffer.writeShort(TlvCode.SDNI_LINK_STATE);
        buffer.writeShort(sdniData.length());
        buffer.writeBytes(sdniData.getBytes());
        ByteBuf finalBuffer = Unpooled.copiedBuffer(this.linkNlri);
        finalBuffer.writeShort(TlvCode.SDNI_LINK_STATE);
        parser.parseNlri(finalBuffer, this.nextHop, builder);*/
        ...
        LOG.trace("local.getCRouterIdentifier: "+local.getCRouterIdentifier());
